...
// Helper functions
async function getAppRoleToken(this: IExecuteFunctions, credentials: any): Promise<string> {
	try {
		const loginData = {
		...
		const config: AxiosRequestConfig = {
			method: 'POST',
			url: `${credentials.url}/v1/auth/approle/login`,
			data: loginData,
			headers: {
		...
		const response: AxiosResponse<VaultAppRoleLoginResponse> = await axios(config);
...
async function readSecret(this: IExecuteFunctions, config: AxiosRequestConfig, secretEngine: string, credentials: any, itemIndex: number): Promise<any> {
	...
	try {
		const response: AxiosResponse<VaultSecretResponse> = await axios({
			...config,
			method: 'GET',
			url,
		});
...
async function writeSecret(this: IExecuteFunctions, config: AxiosRequestConfig, secretEngine: string, credentials: any, itemIndex: number): Promise<any> {
	const secretPath = this.getNodeParameter('secretPath', itemIndex) as string;
...
async function deleteSecret(this: IExecuteFunctions, config: AxiosRequestConfig, secretEngine: string, credentials: any, itemIndex: number): Promise<any> {
	const secretPath = this.getNodeParameter('secretPath', itemIndex) as string;
	const apiVersion = credentials.apiVersion as string;
...
async function listSecrets(this: IExecuteFunctions, config: AxiosRequestConfig, secretEngine: string, itemIndex: number): Promise<any> {
